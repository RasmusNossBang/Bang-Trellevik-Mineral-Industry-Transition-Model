...
*NR(T)..             PI(T) =E= 0.0005*(X('HAV',T)-X('HAV',T+1))+PMAX/(1+PC*sum(I,X(I,T)-X(I,T+1))/H)*(X('HAV',T)-X('HAV',T+1)) - C('HAV')*U('HAV',T)/(1+X('HAV',T)*A('HAV'))*H - F('HAV')*INV('HAV',T)**E('HAV')*H;
...
NPVTID2(T+1)..      NPVT2(T+1) =E= NPVT2(T) + (1/(1+DISK))**((ORD(T)-1)*H)*(0.0005*(X('HAV',T)-X('HAV',T+1))+PMAX/(1+PC*sum(I,X(I,T)-X(I,T+1))/H)*(X('HAV',T)-X('HAV',T+1)) - C('HAV')*U('HAV',T)/(1+X('HAV',T)*A('HAV'))*H - F('HAV')*(INV('HAV',T)**E('HAV'))*H);
NPVTID(T+1)..       NPVT(T+1) =E= NPVT1(T) + (1/(1+DISK))**((ORD(T)-1)*H)*(PMAX/(1+PC*sum(I,X(I,T)-X(I,T+1))/H)*(X('LAND',T)-X('LAND',T+1)) - C('LAND')*U('LAND',T)/(1+X('LAND',T)*A('LAND'))*H - F('LAND')*INV('LAND',T)**E('LAND')*H) + NPVT2(T) + (1/(1+DISK))**((ORD(T)-1)*H)*(0.0005*(X('HAV',T)-X('HAV',T+1))+PMAX/(1+PC*sum(I,X(I,T)-X(I,T+1))/H)*(X('HAV',T)-X('HAV',T+1)) - C('HAV')*U('HAV',T)/(1+X('HAV',T)*A('HAV'))*H - F('HAV')*INV('HAV',T)**E('HAV')*H);
