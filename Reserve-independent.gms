...
***MONOPOLY*** ACTIVATE WHEN MONOPOLY
NR(T)..             PI(T) =E= PMAX/(1+PC*sum(I,X(I,T)-X(I,T+1))/H)*sum(I,X(I,T)-X(I,T+1)) - sum(I,(C(I)*U(I,T))/(A(I)))*H - (sum(I,F(I)*INV(I,T)**E(I)))*H;
...
*NR(T)..             PI(T) =E= PMAX/(1+PC*sum(I,X(I,T)-X(I,T+1))/H)*(X('LAND',T)-X('LAND',T+1)) - C('LAND')*U('LAND',T)/(A('LAND'))*H - F('LAND')*INV('LAND',T)**E('LAND')*H;

********HAV******** ACTIVATE WHEN MAX OCEAN GIVEN WHAT LAND DOES
*NR(T)..             PI(T) =E= PMAX/(1+PC*sum(I,X(I,T)-X(I,T+1))/H)*(X('HAV',T)-X('HAV',T+1)) - C('HAV')*U('HAV',T)/(A('HAV'))*H - F('HAV')*INV('HAV',T)**E('HAV')*H;
...
NPVTID1(T+1)..      NPVT1(T+1) =E= NPVT1(T) + (1/(1+DISK))**((ORD(T)-1)*H)*(PMAX/(1+PC*sum(I,X(I,T)-X(I,T+1))/H)*(X('LAND',T)-X('LAND',T+1)) - C('LAND')*U('LAND',T)/(A('LAND'))*H - F('LAND')*INV('LAND',T)**E('LAND')*H);
NPVTID2(T+1)..      NPVT2(T+1) =E= NPVT2(T) + (1/(1+DISK))**((ORD(T)-1)*H)*(PMAX/(1+PC*sum(I,X(I,T)-X(I,T+1))/H)*(X('HAV',T)-X('HAV',T+1)) - C('HAV')*U('HAV',T)/(A('HAV'))*H - F('HAV')*(INV('HAV',T)**E('HAV'))*H);
NPVTID(T+1)..       NPVT(T+1) =E= NPVT1(T) + (1/(1+DISK))**((ORD(T)-1)*H)*(PMAX/(1+PC*sum(I,X(I,T)-X(I,T+1))/H)*(X('LAND',T)-X('LAND',T+1)) - C('LAND')*U('LAND',T)/(A('LAND'))*H - F('LAND')*INV('LAND',T)**E('LAND')*H) + NPVT2(T) + (1/(1+DISK))**((ORD(T)-1)*H)*(PMAX/(1+PC*sum(I,X(I,T)-X(I,T+1))/H)*(X('HAV',T)-X('HAV',T+1)) - C('HAV')*U('HAV',T)/(A('HAV'))*H - F('HAV')*INV('HAV',T)**E('HAV')*H);

** SPECIAL CONSTRAINTS **
CON1(I,T)..        U(I,T) =L= A(I)*K(I,T);
